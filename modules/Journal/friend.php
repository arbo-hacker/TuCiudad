...
if (is_user($user)) {
    ...
    $result = $db->sql_query($sql);
    $row = $db->sql_fetchrow($result);
    $yn = stripslashes(FixQuotes(check_html(removecrlf($row[name]))));
    $yun = stripslashes(FixQuotes(check_html(removecrlf($row[username]))));
    $ye = stripslashes(FixQuotes(check_html(removecrlf($row[user_email]))));
}
if ($yn == "") {
    $yn = $yun;
}

...
function automated_news() {
	...
	if (($date[1] <= $year) AND ($date[2] <= $month) AND ($date[3] <= $day)) {
		...
		while ($row2 = $db->sql_fetchrow($result2)) {
		    ...
		    if ($num == 0) {
		        $sql = "INSERT INTO ".$prefix."_stories VALUES (NULL, '$catid2', '$aid2', '$title', '$time2', '$hometext', '$bodytext', '0', '0', '$topic2', '$informant2', '$notes', '$ihome2', '$alanguage2', '$acomm2', '0', '0', '0', '0', '$associated2')";
		    }
    		    $db->sql_query($sql);

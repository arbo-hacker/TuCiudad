...
global $prefix, $db;
$aid = substr("$aid", 0,25);
$row = $db->sql_fetchrow($db->sql_query("SELECT radminarticle, radminsuper FROM " . $prefix . "_authors WHERE aid='$aid'"));
if (($row['radminarticle'] == 1) OR ($row['radminsuper'] == 1)) {
...
function removeStory($sid, $ok=0) {
    ...
    $aid = substr("$aid", 0,25);
    $result = $db->sql_query("select counter, radminarticle, radminsuper from ".$prefix."_authors where aid='$aid'");

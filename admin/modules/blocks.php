...
function BlocksEdit($bid) {
    ...
    echo "<br>";
    $bid = intval($bid);
    $row = $db->sql_fetchrow($result = $db->sql_query("select bkey, title, content, url, bposition, weight, active, refresh, blanguage, blockfile, view, expire, action, subscription from ".$prefix."_blocks where bid='$bid'"));
    $bkey = $row['bkey'];
    $title = $row['title'];
    $content = $row['content'];
    $url = $row['url'];
